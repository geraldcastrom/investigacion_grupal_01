...
actividad_diaria$Date <-format(as.Date(actividad_diaria$ActivityDate, format = "%m/%d/%Y"), "%m/%d/%Y")
sueno_diario$Date <- format(as.Date(sueno_diario$SleepDay, format = "%m/%d/%Y"), "%m/%d/%Y")
peso_eventual$Date <- format(as.Date(peso_eventual$Date, format = "%m/%d/%Y"), "%m/%d/%Y")
```
...
write.csv(peso_eventual, "peso_eventual", row.names = FALSE)
write.csv(rc_segundos, "rc_segundos", row.names = FALSE)
```
